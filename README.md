...
| `trello_bulk_fetch_board` | Вложения+комментарии по всей доске (опц. скачать, параллельно) | read |
| `trello_extract_archive` | Скачать и распаковать zip-вложение | read |
| `trello_contact_sheet` | Контактный лист (montage) из набора картинок | util |
...
**Кэш вложений:** скачивание кэшируется (`.trello_cache.json` в папке загрузок).
Если файл уже скачан и не менялся (совпадают id + размер + дата), он не
перекачивается — ответ содержит `"cached": true`. Экономит время и трафик на
больших вложениях.
...
Когда в плагине появились изменения — обнови одной командой (она делает
`git pull` **и** доустанавливает новые зависимости), затем перезапусти Claude:

```bash
# Windows (в папке Trello-Card-Work):
...
После — полностью перезапусти Claude (из трея / `Cmd+Q`) и набери `/mcp`.
...
В папке `skills/` — навыки, которые Claude подхватывает при установке как плагин:

- **`trello-attachments`**, **`trello-design`** — наши (двухэтапный процесс
  Trello → ТЗ → дизайн).
- **`brandkit`**, **`canvas-design`**, **`taste-skill`** — сторонние
  дизайн-скиллы, добавлены для Этапа 2 (отрисовка). Источники и лицензии — в
